...
\subsubsection{Production (Online)}
...
\begin{stepbox}
\textbf{Step 1:} Open a terminal in the project folder
\end{stepbox}

\begin{stepbox}
\textbf{Step 2:} Run \texttt{npm install} (first time only)
\end{stepbox}

\begin{stepbox}
\textbf{Step 3:} Run \texttt{npm run dev} to start the development server
\end{stepbox}

\begin{stepbox}
\textbf{Step 4:} Access the admin panel at \textbf{\url{http://localhost:3000/admin}}
\end{stepbox}

\begin{tipbox}
\textbf{Development Server Architecture:}
\begin{itemize}[noitemsep]
    \item \textbf{Port 3000} --- Next.js website and admin UI
    \item \textbf{Port 4001} --- TinaCMS Vite dev server (internal)
    \item \textbf{Port 9000} --- TinaCMS GraphQL data layer (internal)
\end{itemize}
You only need to access port 3000; the other ports are used internally by the CMS.
\end{tipbox}
